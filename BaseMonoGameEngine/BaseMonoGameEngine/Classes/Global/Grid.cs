...
namespace BaseMonoGameEngine
{
    ...
    public class Grid : IPosition
    {
        ...
        public Grid(in int columns, in int rows, in Vector2 cellSize)
        {
            ...
            GridElements = new List<IPosition>(Columns * Rows);
        }
        ...
        public void AddGridElement(in IPosition element)
        {
            if (element == null)
            {
                Debug.LogError($"Attempting to add null {nameof(element)} to the {nameof(Grid)}!");
                return;
            }
            ...
            {
                Debug.LogWarning($"The {nameof(Grid)} has {NumElementsInGrid} elements which exceeds the max of {MaxElementsInGrid}. "
