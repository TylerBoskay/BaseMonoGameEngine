...
namespace BaseMonoGameEngine
{
    ...
    public struct Line : IEquatable<Line>
    {
        ...
        public Line(in Vector2 p1, in Vector2 p2)
        {
            P1 = p1;
            P2 = p2;
        }

        public Line(in float x1, in float y1, in float x2, in float y2) : this (new Vector2(x1, y1), new Vector2(x2, y2))
        {
        ...
        public float GetLineAngle(in bool inDegrees)
        {
            Vector2 point1 = P1;
            Vector2 point2 = P2;
        ...
        public static Line? GetLineOverlap(in Line line1, in Line line2)
        {
            ...
            Vector2 min1 = new Vector2(Math.Min(line1.P1.X, line1.P2.X), (float)Math.Min(line1.P1.Y * invertY, line1.P2.Y * invertY));
            Vector2 max1 = new Vector2(Math.Max(line1.P1.X, line1.P2.X), (float)Math.Max(line1.P1.Y * invertY, line1.P2.Y * invertY));

            Vector2 min2 = new Vector2(Math.Min(line2.P1.X, line2.P2.X), (float)Math.Min(line2.P1.Y * invertY, line2.P2.Y * invertY));
            Vector2 max2 = new Vector2(Math.Max(line2.P1.X, line2.P2.X), (float)Math.Max(line2.P1.Y * invertY, line2.P2.Y * invertY));

            Vector2 minIntersection;
            if (isDescending)
                minIntersection = new Vector2(Math.Max(min1.X, min2.X), (float)Math.Min(min1.Y * invertY, min2.Y * invertY));
            else
                minIntersection = new Vector2(Math.Max(min1.X, min2.X), (float)Math.Max(min1.Y * invertY, min2.Y * invertY));

            Vector2 maxIntersection;
            if (isDescending)
                maxIntersection = new Vector2(Math.Min(max1.X, max2.X), (float)Math.Max(max1.Y * invertY, max2.Y * invertY));
            else
                maxIntersection = new Vector2(Math.Min(max1.X, max2.X), (float)Math.Min(max1.Y * invertY, max2.Y * invertY));
